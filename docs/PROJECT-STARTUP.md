...
M5 生图闭环的 Worker 合同、失败回滚规则和人工测试步骤见 `docs/M5-IMAGE-GENERATION.md`。同步图片结果必须先经过 MIME/大小校验并原子落盘，再在同一 SQLite 事务中提交资产、结果和任务状态。
...
普通会话输出只保存在对话历史中。用户明确点击“保存为文档草稿”后，系统创建可审阅的工作版本和 Agent 任务记录；只有用户显式发布后，才创建/更新正式权威版本。文档工具栏的“保存并发布”把这两步合并为一次点击，Worker 仍在一个事务内创建并批准审核记录，审核留痕与发布前冲突校验不变。记忆和生产约束仍需用户分别确认保存。
