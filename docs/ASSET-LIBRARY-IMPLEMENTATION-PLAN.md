...
> **版本：**v1.0　 **状态：**已确认
> **负责人：**研发 / 测试 / 产品　 **适用范围：**D:\SB\xlngaiforwindows
...
App 负责素材库工作区路由与 selectedAssetId 深链；ProductionPanel 只保留当前生成结果预览和“查看素材库”入口，不继续维护完整素材列表。列表查询状态放在 AssetLibraryView，避免 ProductionPanel 继续扩大。

- 网格使用稳定的 aspect-ratio 和固定元数据区高度，避免别名和标签导致卡片跳动。
- 图片使用缩略图缓存；视频首期显示封面并在详情中播放，后续再增加悬停预览。
...
| 别名搜索   | alias LIKE，输入 250ms 防抖                        | 数据量或语言需求增长后评估 FTS5    |
| 标签筛选   | 关联表索引 + GROUP BY/HAVING                       | 使用 EXPLAIN QUERY PLAN 做回归门禁 |
...
- 多标签 AND 查询、0 命中、重复标签、稳定排序和分页边界。
