...
版本：1.0
日期：2026-09-19
状态：已实施（P0–P6 核心功能及测试门禁已全部完成；P7 检索核源与标题优先已完成）
...
- [x] 查询分析：抽出《书名》、「引号」、第 N 章/第一章、角色名等身份词，去掉“根据…生成剧本”这类任务套话
- [x] 标题、章号、文件名、书名号命中排在正文 FTS 之前；正文只提到章号的文档不得压过真正的「第一章」
- [x] `library.search` 结果增加 `matchKind`、`kindLabel`、`sourceTypeLabel`，区分标题命中与正文命中，并给出中文类型
- [x] 系统提示要求核源：多候选或标题/章号不符时列出候选、禁止写入；连续章节范围除外
- [x] 检索仍只限当前项目，不扫操作系统、不跨项目

完成标准：`林澈的角色设定` 命中标题「林澈」；`根据第一章生成剧本` 命中小说章节「第一章」而非正文提到第一章的其他文档。
...
| 2026-09-22 | P7：标题/章号/书名号优先于正文 FTS；search 结果带 matchKind 与中文类型；会话提示要求核源后再写。 |
