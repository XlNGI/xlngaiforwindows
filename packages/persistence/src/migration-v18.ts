...
export function widenAgentTaskToolCallLimit(database: Database.Database, appliedAt: string): void {
  const foreignKeys = database.pragma('foreign_keys', { simple: true }) as number;
  if (foreignKeys !== 1) throw new Error('v18 migration requires foreign keys to be enabled.');
  ...
  try {
    database.transaction(() => {
      ...
      database.exec('ALTER TABLE agent_tasks RENAME TO __v17_old_agent_tasks');
      database.exec(V18_AGENT_TASKS_SCHEMA);
      database.exec(`
        ...
        FROM __v17_old_agent_tasks;
      `);
      database.exec(V18_TASK_INDEXES_AND_TRIGGERS);
      database.exec('DROP TABLE __v17_old_agent_tasks');
      ...
      if (foreignKeyErrors.length > 0) {
        throw new Error(
          `v18 migration produced ${foreignKeyErrors.length} foreign key violation(s).`,
        );
      }
      const staleReferences = database
        .prepare("SELECT name FROM sqlite_master WHERE sql LIKE '%__v17_old_agent_tasks%'")
        .all() as Array<{ name: string }>;
      if (staleReferences.length > 0) {
        throw new Error(
          `v18 migration left stale schema references: ${staleReferences.map((row) => row.name).join(', ')}`,
        );
      }
      database
...
const V18_TASK_INDEXES_AND_TRIGGERS = `
