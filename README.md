...
2026-08-31 起进入统一 Agent 体验阶段：统一自然语言入口、跨供应商模型选择、动态媒体参数、附件分析/引用和 schema 提议确认链路已接通；项目 Schema v35 已持久化会话级模型偏好、媒体任务快照、Provider 生命周期事件，以及受控 Agent 的 `schema-query` 任务、待确认 Schema 提议和审计查询，并提供项目隔离的事件分页查询、任务日志时间线、重启恢复审计、统一中文媒体状态/错误反馈、视频等待说明和生成后的直接素材入口。普通会话可直接查询适配器参数 Schema、提交受 Worker 校验的 Schema 提议或查询变更历史；低风险字段变更可自动确认，高风险变更可在确认卡片中确认或拒绝并回滚。
