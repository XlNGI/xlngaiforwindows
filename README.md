...
2026-08-31 起进入统一 Agent 体验阶段：统一自然语言入口、跨供应商模型选择、动态媒体参数、附件分析/引用和 schema 提议确认链路已接通；项目 Schema v34 已持久化会话级模型偏好、媒体任务快照和 Provider 生命周期事件，并提供项目隔离的事件分页查询、任务日志时间线、重启恢复审计、统一中文媒体状态/错误反馈、视频等待说明和生成后的直接素材入口。后续重点是 Agent schema 工具循环和其余任务类型的统一状态反馈。
